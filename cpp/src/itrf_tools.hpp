...
struct StationId {
  ...
  const char* domes() const noexcept {return domes; }
};
...
namespace itrf_details {
  ...
  int read_next_record_psd(std::ifstream & psd_stream,
  ...
  int read_next_record(std::ifstream & ssc_stream,
                       ssc_record & record) noexcept;
...
int ssc_extrapolate(std::ifstream &fin,
                    ...
                    std::vector<sta_crd> &results, itrf_details::stationComparissonPolicy policy) noexcept;
